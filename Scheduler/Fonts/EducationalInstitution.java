...
private void SameLevel(Session base, Session currentS, Restriction r, int pos) {
	if (rightSubjectList(currentS.getGS(),r, 4) &&
			base.getGS().getGroup().equals(currentS.getGS().getGroup())) {
		for(int i = 0; i < CR.size(); i++) {
        currentS.setBits(60*i+(pos%60),60*i+(pos%60)+base.getGS().getSessionLength()+1,false); // Is that +1 necesary?
    }
	}
}
...
private void SSTsameHour(Session currentS, Restriction r, int pos, int j, Vector<Restriction> restric) {
  if (rightSubject(currentS.getGS(),r) && r.getParameter(5).equals(currentS.getGS().getSessionType()) && numImposedSession(j,r.getParameter(0),currentS.getGS(),restric) == currentS.getID()) {
    for (int i = 0; i < CR.size(); i++) {
      ...
      //System.out.println("1: " + (i*60+pos%60));
      //System.out.println("2 " + (i*60+pos%60+currentS.getGS().getSessionLength()));
      currentS.setBits(i*60+pos%60+currentS.getGS().getSessionLength(),(i+1)*60,false); // look if in still sat looks forward not in same classroom
    }
  }
}
...
//Main function to output schedules by Classroom
public void printSchedule(){
  System.out.println("-----------------------------------------");
  System.out.println("A schedule was successfully generated!");
  System.out.println("-----------------------------------------");
  String selectedClassroom = new String("");

  while( !selectedClassroom.equals("END") ){

    switch(selectedClassroom) {
      case "ALL":
        System.out.println("---------------------------------------------------");
        System.out.println("\tPrinting schedules of all classrooms");
        System.out.println("---------------------------------------------------");
        for (int j = 0 ; j < CR.size(); j++){
          Classroom auxClassroom = CR.get(j);
          System.out.println("Timetable for classroom " + auxClassroom.getName() + ":");
          for( int i = 0; i < 60; i++ ){
            if( !(auxClassroom.getSlotInfo(i) == null) ){
              String output = getTimeFrame(i) + auxClassroom.getSlotInfo(i).getInfo();
      ...
      case "": break;
      default: {
        Classroom auxClassroom = this.getClassroom(selectedClassroom);
    ...
    System.out.println("Please, enter a classroom name to print its timetable, ALL to print them all or End to exit.");
    selectedClassroom = in.nextLine();
  }
  System.out.println("-------------------------------------------------");
  System.out.println("\tExiting schedule printer");
  System.out.println("-------------------------------------------------");
}
...
// Output when INSAT
public void scheduleUnsatisfiable(){
  System.out.println("-----------------------------------------");
...
public void generateSchedule(Scanner sc) {
	in = sc;
  //DomainControl domainControl = DomainControl.getInstance();
  ...
  while(i < sessions.size() && i != -1 && limit < 1000000)
  {
    limit++;
    int pos = -1;
    ...
    {
      // domainControl.signalTimeSchedule("Assigning " + getTimeFrame(pos));
      // domainControl.signalTimeSchedule("Assigning " + currentS.getInfo));
      // domainControl.signalTimeSchedule("Assigning " + currentS.getGS().getSubject() + "... " + getTimeFrame(pos));
